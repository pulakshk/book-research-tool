...
async def extract_series_info(page: Page) -> Dict[str, Optional[str]]:
    """
    Extract series name and URL from Goodreads book page
    """
    result = {
        'series_name': None,
        'series_url': None
    ...
    try:
        # Strategy 1: Modern h3 header (Most reliable)
        # <h3 class="Text__title3" aria-label="Book Series"><a ...>Series Name #1</a></h3>
        series_elem = await page.query_selector('h3[aria-label="Book Series"] a')
        
        # Strategy 2: Fallback for older layouts or sub-headers
        if not series_elem:
            series_elem = await page.query_selector('a[href*="/series/"]')
            
        if series_elem:
            text = (await series_elem.text_content()).strip()
            href = await series_elem.get_attribute('href')
            
            if text:
                result['series_name'] = text
            if href:
                result['series_url'] = href if href.startswith('http') else f"https://www.goodreads.com{href}"
                
        # Strategy 3: Parse from Title (Fallback if metadata missing)
        # e.g., "Hockey Star’s Unexpected Twins (The Billionaire Rules Series)"
        if not result['series_name']:
            ...
            if title_elem:
                ...
                # Pattern: Title (Series Name, #1) or Title (Series Name)
                # Matches: "(The Billionaire Rules Series)"
                match = re.search(r'\(([^)]+Series[^)]*)\)', title_text, re.IGNORECASE)
                if match:
                    result['series_name'] = match.group(1).strip()
                else:
                    # Try detection of "Book X of Y" or similar if needed
                    # Or generic paren extract if it looks like a series
                    match_generic = re.search(r'\(([^)]+)\)', title_text)
                    if match_generic:
                        candidate = match_generic.group(1).strip()
                        # Simple heuristic: if it has "Series" or a number with #
                        if 'series' in candidate.lower() or '#' in candidate:
                            result['series_name'] = candidate
...
async def extract_goodreads_comprehensive(page: Page) -> Dict[str, any]:
    ...
    # Extract Series Info
    series_info = await extract_series_info(page)
    ...
    return {
        ...
        'series_url': series_info['series_url'],
